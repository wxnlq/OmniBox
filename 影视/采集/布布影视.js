...
const logInfo = (message, data = null) => {
    ...
    OmniBox.log("info", `[布布影视] ${output}`);
};

const logError = (message, error) => {
    OmniBox.log("error", `[布布影视] ${message}: ${error.message || error}`);
};
...
// ============================================================
// WASM 解码模块 - 用于解密 bubuyingshi.com 的加密播放地址
...
async function detail(params) {
    ...
    try {
        ...
        rawShows.forEach((showCode, index) => {
            ...
            if (urls.length > 0) {
                validLines.push({
                    showCode: showCode, // 【修改点1】：将线路代码保存下来用于排序
                    lineName,
                    playUrls: urls.join('#'),
        ...
        // 【修改点2】：自定义线路排序优先级
        const SHOW_CODE_PRIORITY =['YYNB', 'BBA', 'NBY', 'JD4K'];

        validLines.sort((a, b) => {
            const codeA = (a.showCode || "").toUpperCase();
            const codeB = (b.showCode || "").toUpperCase();
            
            const indexA = SHOW_CODE_PRIORITY.indexOf(codeA);
            const indexB = SHOW_CODE_PRIORITY.indexOf(codeB);
            
            // 如果两者都在优先级列表中，按指定的顺序（索引）排序
...
async function play(params) {
    ...
    try {
        ...
        const play_from = parts[0];              // 线路代码 (YYNB, BBA, NBY, JD4K...)
