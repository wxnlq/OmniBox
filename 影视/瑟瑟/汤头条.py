...
def _extract_cover(content):
    if not content:
        return ""
    patterns = (
        r'(?:data-cover|data-original|data-src|poster)=["\']([^"\']+)',
    ...
    for pattern in patterns:
        ...
        if match:
            url = htmllib.unescape(match.group(1).strip())
            if url.startswith("//"):
                return "https:" + url
...
def _pack_video_id(video_id, cover):
    if not cover:
        return video_id
    return f"{video_id}#cover={urllib.parse.quote(cover, safe='')}"


def _unpack_video_id(value):
    value = str(value or "").strip()
    if "#cover=" not in value:
        return value, ""
    video_id, encoded_cover = value.split("#cover=", 1)
    return video_id, urllib.parse.unquote(encoded_cover)
...
class Spider(Spider):
    ...
    def detailContent(self, ids, cover=""):
        try:
            did = str(ids[0] if isinstance(ids, (list, tuple)) else ids).strip()
        except:
            return {"list": []}
        did, packed_cover = _unpack_video_id(did)
        cover = cover or packed_cover
        if not did:
            return {"list": []}
        ...
        h1_match = re.search(r"<h1\b[^>]*>(.*?)</h1\s*>", html, re.I | re.S)
        title_source = title_match or h1_match
        if title_source:
            title = _clean(title_source.group(1))
            title = re.split(r"\s*[|｜]\s*|\s+[-–—]\s+|\s+_\s*", title, maxsplit=1)[0].strip()

        pic = cover or self._cover_cache.get(did, "")
        if not pic:
            pic = self._cover_cache.get(_cover_key(did), "")
    ...
    def _parse_list(self, html):
        ...
        for m in re.finditer(r'href="(/video/(\d+)/)"[^>]*title="([^"]+)"', html):
            ...
            if not pic:
                ...
                cm = _extract_cover(chunk)
                if cm:
                    pic = cm
            videos.append({
                "vod_id": _pack_video_id(self.host + href, pic),
                "vod_name": title,
                "vod_pic": pic,
                "vod_remarks": "在线",
            })
            self._cover_cache[self.host + href] = pic
            self._cover_cache[_cover_key(self.host + href)] = pic
            self._cover_cache[vid] = pic
        if videos:
            return videos
...
async def home(params, context):
    await OmniBox.log("info", "加载汤头条首页")
    return _get_spider().homeContent((params or {}).get("filter", False))


async def category(params, context):
    params = params or {}
    return _get_spider().categoryContent(
        params.get("categoryId", params.get("type_id", "home")),
        params.get("page", 1),
        params.get("filter", False),
        params.get("extend", {}),
    )
...
async def detail(params, context):
    params = params or {}
    video_id = params.get("videoId", params.get("id", ""))
    cover = params.get("vod_pic", params.get("vodPic", ""))
    result = _get_spider().detailContent([video_id], cover)
    for vod in result.get("list", []):
        if cover:
            vod["vod_pic"] = cover
        play_url = vod.pop("vod_play_url", "")
        play_from = vod.pop("vod_play_from", "线路1")
        episodes = []
        for episode in play_url.split("#"):
            if "$" in episode:
                name, play_id = episode.split("$", 1)
                episodes.append({"name": name, "playId": play_id})
        vod["vod_play_sources"] = [{"name": play_from, "episodes": episodes}]
    return result


async def search(params, context):
    params = params or {}
    return _get_spider().searchContent(
        params.get("key", ""),
        params.get("quick", False),
        params.get("page", 1),
    )
...
async def play(params, context):
    params = params or {}
    play_id = params.get("playId", params.get("id", ""))
    result = _get_spider().playerContent(
        params.get("flag", "play"),
        play_id,
        params.get("vipFlags", []),
    )
    return {
        "urls": [{"name": "播放", "url": result.get("url", "")}],
        "flag": params.get("flag", "play"),
        "header": result.get("header", {}),
        "parse": result.get("parse", 0),
    }
